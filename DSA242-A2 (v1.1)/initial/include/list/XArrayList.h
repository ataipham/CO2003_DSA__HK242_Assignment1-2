...
protected:
    ...
    /** equals:
     * if T: primitive type: int, double, string, char, bool, etc:
...
{
    checkIndex(index); //throw exception if index is invalid
    T res = data[index];
...
bool XArrayList<T>::removeItem(T item, void (*removeItemData)(T))
{
    ...
    if(removeItemData != nullptr) removeItemData(data[idx]);
    removeAt(idx);
    return true;
}
...
string XArrayList<T>::toString(string (*item2str)(T &))
{
    ...
    for(Iterator it = begin(); it != end(); it++){
        if(it != begin()) ss << ", ";
        if(item2str != nullptr) ss << item2str(*it);
        else ss << *it;
    }
    ss << "]";
    return ss.str();
