...
void Heap<T>::remove(T item, void (*removeItemData)(T)){
    ...
    if (foundIdx > 0 && aLTb(elements[foundIdx], elements[parent]))   // for min‑heap
        reheapUp(foundIdx);
    else
        reheapDown(foundIdx);
     //free item's memory
...
void Heap<T>::reheapDown(int position){
    ...
    if(leftChild < count && aLTb(this->elements[leftChild], this->elements[smallest]))
        smallest = leftChild;
    if(rightChild < count && aLTb(this->elements[rightChild], this->elements[smallest]))
        smallest = rightChild;
...
void Heap<T>::copyFrom(const Heap<T>& heap){
    ...
    this->deleteUserData = 0; //do not copy deleteUserData
